...
class NLPNERDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        word_ids = [
            self.config.word2idx.get(w, self.config.word2idx[self.config.UNK])
            for w in words
        ]
        ner_tag_ids = [
            self.config.label2idx.get(t, self.config.label2idx[self.config.O])
            for t in ner_tags
        ]
...
def nlp_collate_fn(batch, config):
    ...
    word_seq_tensor = pad_sequence(
        word_sequences, batch_first=True, padding_value=config.word2idx[config.PAD]
    )
    label_seq_tensor = pad_sequence(
        label_sequences, batch_first=True, padding_value=config.label2idx[config.PAD]
    )
...
def load_umt_data():
    ...
    def generate_sentiment_labels(sentences):
        ...
        for sent in sentences:
            text = " ".join(sent)
            sentiment = get_sentiment_binary(text)
